...
| Has N items with shared attributes | **Grid** (card grid or data table — see below) |
| Needs structured data display | **Table** (grid_table with colored headers) |

### When to use Grid layouts

Grids are the right pattern when you have **3+ items that share the same attributes** and the viewer needs to scan/compare them. Two flavors:

**Card grid** (`card_grid()` from `references/charts.py`): styled cards in rows × cols. Each card has a colored header, body text, and optional accent stripe. Use for:
- Feature comparisons (3-5 products/tools side by side)
- Pattern catalogs (like prompt engineering patterns)
- Multi-concept overviews where each concept deserves its own visual region

**Data table** (`grid_table()` from `references/charts.py`): header row + body rows with cell borders. Use for:
- Structured specs (model comparison, API endpoints, config options)
- Any content that is naturally tabular (rows × columns of data)

Grid helpers accept `stroke_style` ("solid", "dashed", "dotted") and custom colors for headers/cells.

**When NOT to use grids**: if items have parent-child relationships (use tree), if items form a sequence (use timeline/assembly), or if there are only 2 items (use side-by-side).

### Step 3: Ensure Variety
For multi-concept diagrams: **each major concept must use a different visual pattern**. Uniform card grids for an entire diagram are still wrong — but a grid section alongside timeline, fan-out, and evidence sections is excellent.
...
Most native Excalidraw users work on dark canvas. Default light-mode colors (e.g. `#3b82f6` bright blue) wash out against `#0f172a` background. Switch to the dark palette in `color-palette.md` when:

- User says "dark mode" / "dark canvas" / "dark theme"
- User shares a reference with dark `viewBackgroundColor`
- Output target is video/slides on dark theme

**To switch a diagram to dark mode:**

1. Set `appState.viewBackgroundColor: "#0f172a"` (instead of `#ffffff`)
2. Use **dark palette** for all shape fills/strokes (inverted: dark fill + bright stroke)
3. Use **light text** colors: titles `#f1f5f9`, body `#94a3b8`
4. Evidence artifacts can use deeper bg `#020617` and brighter green `#4ade80`

Don't mix: a diagram is fully light OR fully dark, not both.
...
**Default to 0** for most professional use cases. Use `roughness: 1` when you want warmth or an informal feel.

### Fill Style (visual texture inside shapes)
- `fillStyle: "solid"` — Flat color. Default for clean diagrams.
- `fillStyle: "hachure"` — Diagonal hatching lines. Adds hand-drawn texture. Good for supporting shapes.
- `fillStyle: "cross-hatch"` — Cross-hatched grid pattern. Creates visual weight. Good for hero/emphasis shapes.

**Use fill styles for visual hierarchy within a diagram**: hero shapes get `cross-hatch`, supporting shapes get `hachure`, background/subtle shapes get `solid`. Don't use the same fill style for everything.

### Stroke Style (lines and arrows)
- `strokeStyle: "solid"` — Default. Primary flow and connections.
- `strokeStyle: "dashed"` — Optional paths, async flow, feedback loops, "maybe" relationships.
- `strokeStyle: "dotted"` — Weak connections, annotations, secondary detail.

**Mix stroke styles in one diagram** to create visual depth: primary flow = solid, feedback = dashed, annotations = dotted.

### Arrow Heads
- `endArrowhead: "arrow"` — Filled triangle (default). Shows direction.
- `endArrowhead: "bar"` — Flat bar. Shows termination/boundary.
- `endArrowhead: "dot"` — Circle. Shows connection point.
- `endArrowhead: "triangle"` — Open triangle. Lighter directional hint.
- `endArrowhead: null` — No head. For structural lines.

Use `startArrowhead` for bidirectional arrows.
...
Use these helpers instead of computing coordinates and merging JSON by hand.

### Coordinate helpers — `references/layout.py`

Pure-Python helpers for common patterns. Import or run inline:

- `fan_out(center, n, radius, angle_span, start_angle)` — radial points around a hub
- `timeline(y, x_start, x_end, n)` — evenly-spaced markers on a horizontal line
- `tree(root_xy, n_branches, level_height, branch_width)` — trunk + branch coordinates
- `side_by_side(left_x, right_x, y, width, height)` — parallel column layout
- `section(index, y_start, content_height)` — title/divider/content y-coords for a numbered section
- `stack(start_xy, item_sizes, direction, gap)` — stack elements with consistent gap

Use these for any pattern with >2 elements. Hand-computed coords drift; helpers don't.

### Pre-render lint — `references/lint.py`

Run before every render to catch issues that the render-validate loop would otherwise burn iterations on:

```bash
cd .claude/skills/excalidraw-diagram/references && uv run python lint.py path/to/file.excalidraw
```

Checks: bbox overlap, text-overflow inside containers, arrows pointing into nothing, container ratio (>30% triggers warning), required fields. Exit 0 if clean.

### Section-by-section merging — `references/append_section.py`

For comprehensive diagrams, build sections in separate files and merge:

```bash
uv run python append_section.py base.excalidraw new_section.json
```
...
- `examples/fan-out.excalidraw`
- `examples/timeline.excalidraw`
- `examples/tree.excalidraw`
- `examples/side-by-side.excalidraw`
- `examples/assembly-line.excalidraw`
- `examples/convergence.excalidraw`
...
**0. Lint first** — Before rendering, run `uv run python lint.py <file>` from the references directory. Fix any reported issues (overlaps, text overflow, dangling arrows, excessive containers) before burning a render cycle. Lint exits 0 when clean.
